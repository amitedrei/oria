...
def get_image_emotions(image: UploadFile):
    image_path = None
    try:
        image_path = save_upload_file(image, (image.filename or "").split(".")[-1])
        ...
        emotion_probs = {
            emotion: float(probs[i]) for i, emotion in enumerate(EMOTION_LABELS)
        }
        print(f"Image emotions: {emotion_probs}")
...
def get_image_text(image: UploadFile) -> str:
    image_bytes = image.file.read()
    image_object = Image.open(BytesIO(image_bytes)).convert("RGB")
    image_object = image_object.resize((128, 128))
    preds = image_to_text(image_object)
