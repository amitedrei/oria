...
@router.post("/image-to-emotions")
async def image_to_emotions(image: UploadFile = File(...)):
    if not (image.content_type or "").startswith("image/"):
        raise HTTPException(
            status_code=400,
            detail="File must be an image",
        )
    return get_image_emotions(image)
...
async def image_to_text(image: UploadFile = File(...)):
    ...
    return get_image_text(image)


@router.post("/text-to-embeddings")
async def text_to_embeddings(data: TextToEmbeddingsModel):
    if not data.text:
